 ...
 ******************************************************************************
 ...
 The library is a single-bus temperature sensor, such as DS18B20 series
...
Dallas_Temperature::Dallas_Temperature(OneWire* _oneWire)
#if REQUIRESALARMS
...
bool Dallas_Temperature::validAddress(const uint8_t* deviceAddress)
{
...
bool Dallas_Temperature::isConnected(const uint8_t* deviceAddress)
{
    ScratchPad scratchPad;
...
bool Dallas_Temperature::isConnected(const uint8_t* deviceAddress, uint8_t* scratchPad)
{
    readScratchPad(deviceAddress, scratchPad);
...
void Dallas_Temperature::readScratchPad(const uint8_t* deviceAddress, uint8_t* scratchPad)
{
    // send the command
    _wire->reset();
...
void Dallas_Temperature::writeScratchPad(const uint8_t* deviceAddress, const uint8_t* scratchPad)
{
    _wire->reset();
    _wire->select(deviceAddress);
...
bool Dallas_Temperature::readPowerSupply(const uint8_t* deviceAddress)
{
    bool ret = false;
    _wire->reset();
    _wire->select(deviceAddress);
...
bool Dallas_Temperature::setResolution(const uint8_t* deviceAddress, uint8_t newResolution)
{
    ScratchPad scratchPad;
...
uint8_t Dallas_Temperature::getResolution(const uint8_t* deviceAddress)
{
    // DS1820 and DS18S20 have no resolution configuration register
...
bool Dallas_Temperature::isConversionAvailable(const uint8_t* deviceAddress)
{
...
bool Dallas_Temperature::requestTemperaturesByAddress(const uint8_t* deviceAddress)
{
    _wire->reset();
    _wire->select(deviceAddress);
    _wire->write(STARTCONVO, parasite);
...
int16_t Dallas_Temperature::millisToWaitForConversion(uint8_t bitResolution)
{
...
void Dallas_Temperature::blockTillConversionComplete(uint8_t bitResolution, const uint8_t* deviceAddress)
{
...
// Fetch temperature for device index
float Dallas_Temperature::getTempFByIndex(uint8_t deviceIndex)
{
    DeviceAddress deviceAddress;
    if (!getAddress(deviceAddress, deviceIndex))
...
int16_t Dallas_Temperature::calculateTemperature(const uint8_t* deviceAddress, uint8_t* scratchPad)
{
...
// Dallas_Temperature.h. It is a large negative number outside the
// operating range of the device
int16_t Dallas_Temperature::getTemp(const uint8_t* deviceAddress)
{
    ScratchPad scratchPad;
...
// Dallas_Temperature.h. It is a large negative number outside the
// operating range of the device
float Dallas_Temperature::getTempC(const uint8_t* deviceAddress)
{
...
// Dallas_Temperature.h. It is a large negative number outside the
// operating range of the device
float Dallas_Temperature::getTempF(const uint8_t* deviceAddress)
{
...
void Dallas_Temperature::setHighAlarmTemp(const uint8_t* deviceAddress, char celsius)
{
...
// accepts a float, but the alarm resolution will ignore anything
// after a decimal point.  valid range is -55C - 125C
void Dallas_Temperature::setLowAlarmTemp(const uint8_t* deviceAddress, char celsius)
{
    // make sure the alarm temperature is within the device's range
    if (celsius > 125) celsius = 125;
    else if (celsius < -55) celsius = -55;
...
char Dallas_Temperature::getHighAlarmTemp(const uint8_t* deviceAddress)
{
    ScratchPad scratchPad;
...
// DEVICE_DISCONNECTED for an address
char Dallas_Temperature::getLowAlarmTemp(const uint8_t* deviceAddress)
{
    ScratchPad scratchPad;
...
// Dallas_Temperature::resetAlarmSearch() to start over.
...
bool Dallas_Temperature::hasAlarm(const uint8_t* deviceAddress)
{
    ScratchPad scratchPad;
    if (isConnected(deviceAddress, scratchPad))
    {
...
void Dallas_Temperature::defaultAlarmHandler(const uint8_t* deviceAddress)
{
}
...
float Dallas_Temperature::toFahrenheit(float celsius)
{
    return (celsius * 1.8) + 32;
}

// Convert float Fahrenheit to Celsius
float Dallas_Temperature::toCelsius(float fahrenheit)
{
    return (fahrenheit - 32) * 0.555555556;
}

// convert from raw to Celsius
float Dallas_Temperature::rawToCelsius(int16_t raw)
{
...
float Dallas_Temperature::rawToFahrenheit(int16_t raw)
{
    if (raw <= DEVICE_DISCONNECTED_RAW)
        return DEVICE_DISCONNECTED_F;
    // C = RAW/128
...
// MnetCS - Allocates memory for Dallas_Temperature. Allows us to instance a new object
void* Dallas_Temperature::operator new(unsigned int size) // Implicit NSS obj size
{
    void * p; // void pointer
    p = malloc(size); // Allocate memory
    memset((Dallas_Temperature*)p,0,size); // Initialise memory

    //!!! CANT EXPLICITLY CALL CONSTRUCTOR - workaround by using an init() methodR - workaround by using an init() method
    return (Dallas_Temperature*) p; // Cast blank region to NSS pointer
}

// MnetCS 2009 -  Free the memory used by this instance
void Dallas_Temperature::operator delete(void* p)
{
    Dallas_Temperature* pNss =  (Dallas_Temperature*) p; // Cast to NSS pointer
    pNss->~Dallas_Temperature(); // Destruct the object
